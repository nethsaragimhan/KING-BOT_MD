...
// ════════════════════WA CONNECTION
    ...
    conn.browserDescription = ["KINGBOTMD", "Safari", '1.0.0']
...
${chalk.blue.italic('🇱🇰 Connecting to WhatsApp...▶')}`);
    ...
    conn.on('open', async () => {
        ...
        var Commands = await plugindb.PluginDB.findAll();
        ...
        console.log(
            chalk.green.bold(' KING WHATSAPP BOT WORKING!▷')
       );
        
        console.log(
            chalk.blueBright.italic('KING WhatsApp User Bot V1.0.0')
        );
        
         if (config.LANG == 'EN') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/logo.jpeg"), MessageType.image, { caption: `🙋‍♂️ Hellow !! ${conn.user.name}! \n\n*✨ Welcome To [🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD WhatsApp User Bot  :│✨ *\n\n\n Your Bot Working  As ${config.WORKTYPE} ✨ .\n\n*✨ │[🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD WORKING Your Account*\n\n*✨ ▷ Use the 🚀.menu command to get bot menu...*\n\n\n*✨  [🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD is a powerfull WhatsApp robot developed by ɯԋιƚҽ ԋαƈƙҽɾʂ.*\n\n*🚀 This is your LOG number. Avoid using the command here.\n\n✨ .update Command use for new items*\n\n`});
             
         } else if (config.LANG == 'SI') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/logo.jpeg"), MessageType.image, { caption: `🙋‍♂️ අයුබොවන් !! ${conn.user.name}! \n\n*✨  සාදරයෙන් [🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD WhatsApp User Bot  වෙත පිලිගන්නවා :│✨ *\n\n\n ඔබේ Bot ${config.WORKTYPE} ලෙස ක්‍රියාකරයි.\n\n*✨ │[🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD ඔබගේ ගිණුමේ දැන් සක්‍රියයි*\n\n*✨  [🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD bot සම්පූර්න මෙනුව ලබා ගැනීමට ✨ .alpha විධානය භාවිතා කරන්න...*\n\n\n*✨  [🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD යනූ සීඝ්‍රයෙන් වර්ධනය වන Whatsapp රොබෝවෙකි..[🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD වෙත ලැබෙන නව අංග හා යතාවත්කාලින කිරිම් ලබා ගැනීමට✨ .update විධානය භාවිතා කරන්න..*\n\n*✨  මෙය ඔබගේ LOG අංකයයි.මෙහි විධාන භාවිතයෙන් වළකින්න.*\n\n`});
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            if (conn.user.jid.startsWith('90')) { 
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time + '\n\nPOWERD BY [🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD WhatsApp User Bot '
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/logo.jpeg"), MessageType.image, { caption: '[🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD WhatsApp User Bot  WORKING AS '+config.WORKTYPE+'!!\n\n▷ _This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/logo.jpeg"), MessageType.image, { caption: '[🇱🇰𝚱𝚰𝚴Ｇ 𝛃𝚯𝚪🤘] - MD WhatsApp User Bot   '+config.WORKTYPE+' ලෙස ක්‍රියා කරයි!!\n\n▷ _මෙය ඔබගේ LOG අංකයයි මෙහි විධන භාවිතයෙන් වළකින්න_\n▷ඔබට යම් ගැටලුවක් ඇත්නම් අපගේ සහය සමූහට ලිවිය හැක.\n*දෝෂය:* ```' + error + '```\n\n' });
